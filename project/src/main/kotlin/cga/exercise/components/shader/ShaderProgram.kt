...
class ShaderProgram(vertexShaderPath: String, fragmentShaderPath: String) {
    ...
    fun setUniform(name: String, vector3D : Vector3f) : Boolean
    {
        if (programID == 0)
            return false
        ...
        if (loc != -1)
        {
            GL20.glUniform3fv(loc, floatArrayOf(vector3D.x, vector3D.y, vector3D.z))
            return true
        }
    ...
    fun setUniform(name: String, vector2D : Vector2f) : Boolean
    {
        if (programID == 0)
            return false
        ...
        if (loc != -1)
        {
            GL20.glUniform2fv(loc, floatArrayOf(vector2D.x, vector2D.y))
            return true
        }
